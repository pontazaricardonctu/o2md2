...
void printStatisticsFile(unsigned long long* keyGenTimeArray, unsigned long long* encryptionTimeArray, unsigned long long* decryptionTimeArray) {
    ...
    #ifdef FFT
        snprintf(fileNameBuffer, sizeof(char)*50, "totalStatistics-O2MD2-OH64%dFFT.csv",dimension);
        statsFile = fopen(fileNameBuffer, "w+");
    #else
        snprintf(fileNameBuffer, sizeof(char)*50, "totalStatistics-O2MD2-OH64%d.csv",dimension);
        statsFile = fopen(fileNameBuffer, "w+");
    #endif
